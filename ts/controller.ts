...
export default class Controller {
    public static readonly BALL_SPEED_ADJUSTMENT = .05
    ...
    private _keyState: Record<GameKeyType, boolean> & OptionalIndex<boolean> = { KeyW: false, KeyS: false, ArrowUp: false, ArrowDown: false }
    ...
    public setParams(data: FormData) {
        ...
        const commonParams = {
            ...
            speed: {
                ball: (Number(data.get('ball-speed')) || Controller.BALL_DEFAULT_SPEED) * Controller.BALL_SPEED_ADJUSTMENT,
                racket: (Number(data.get('racket-speed')) || Controller.RACKET_DEFAULT_SPEED) * Controller.RACKET_SPEED_ADJUSTMENT
            }
        }

        switch (data.get('mode')) {
            case 'competitive':
                newParams = Object.assign({}, { mode: ModeType.competitive as const, lives: Number(data.get('lives')) || Controller.DEFAULT_LIVES }, commonParams)
                break
            case 'free':
                newParams = Object.assign({}, { mode: ModeType.free as const, needRestart: !!data.get('need-restart'), hasCounter: !!data.get('has-counter') }, commonParams)
                break
            default: return
        }
        ...
        if (newParams.mode !== Model.params.mode && this._state !== GameStateType.stop) {
            needRestart = true
    ...
    private _moveBall = (time: number) => {
        ...
        if (error instanceof GamePlayerError)
            void this.toggleGameState(error instanceof GameLossError ? GameStateType.stop : GameStateType.miss)
        else
            this._requestID = this._state === GameStateType.play ? requestAnimationFrame(this._moveBall) : undefined
    }
    ...
    private _initControls(controls: IControls & { resume: HTMLCollectionOf<Element> }) {
        ...
        const tryResumeGame = () => {
            ...
            if (needChangeState) {
                this._state === GameStateType.pause && this.toggleGameState(GameStateType.play)
        ...
        for (const element of controls.resume)
            element.addEventListener('click', () => this.toggleGameState(GameStateType.play))

        controls.settingsForm?.addEventListener('reset', () => {
            // Just a hack to execute code after form reset
            setTimeout(() => updateSettingsVisibility(document.querySelector('input:checked[type=radio][name=mode]') as HTMLInputElement))
